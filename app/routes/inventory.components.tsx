...
export default function ComponentsRoute() {
  ...
  const editRecord = (record: Component) => {
    navigate(`/inventory/form-component/${record.id}`);
  };
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const dataFetch = await ComponentService.getAllPosts(isDepartmentID);
  ...
  const handleCheckoutButton = (data: Component) => {
    setIsModalOpen(true);
    setDataRow(data);
  };
  ...
  const columns: TableColumnsType<Component> = [
    ...
    },
    {
      ...
      render: (_, data) => (
        <div>
          {data.components_check[0]?.count >= data.min_qty ? (
            // If count meets or exceeds minimum quantity
            <span className="flex flex-wrap text-green-600">
              (<TiWarning className="mt-1 text-orange-500" />) {data.components_check[0].count}
            </span>
          ) : (
            // If count is below minimum quantity
            <span className="text-green-600">
              {data.components_check[0]?.count || 0}
            </span>
          )}
        </div>
      )
    ...
    },
    {
      ...
      fixed: "right",
      render: (_, data) => {
        const currentCount = data?.components_check?.[0]?.count || 0;
        ...
        return (
          <div>
            <Popconfirm
              ...
              description="Are you sure to checkout this component?"
              okText="Yes"
              cancelText="No"
    ...
    <div className="w-full px-6 py-4 rounded-lg shadow-sm">
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">Check-out Component</span>
          </div>
        }
        open={isModalOpen}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <Breadcrumb
            items={[
              ...
              {
                title: <span className="text-blue-600 font-medium">Technology Components</span>,
              },
            ]}
            className="text-sm"
          />
        ...
        <Space wrap className="mt-2 sm:mt-0">
          <Link to="/inventory/deleted-components">
            <Button
          ...
          <Link to="/inventory/form-component">
            <Button
              icon={<AiOutlinePlus />}
              type="primary"
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6 rounded-lg">
        <Alert
          message="Component Management: Track and manage all technology components in your inventory system."
          type="info"
          showIcon
        />
          ...
          locale={{
            emptyText: (
              <div className="py-8 flex flex-col items-center">
                ...
                <p className="text-gray-500 mb-4">No technology components found</p>
                <Button
                  type="primary"
                  className="mt-2"
                  onClick={() => navigate('/inventory/form-component')}
                  icon={<AiOutlinePlus />}
                >
                  Add First Component
                </Button>
