...
export default function WorkflowLayoutIndex() {
  ...
  useEffect(() => {
    if (currentPath === '/workflow') {
      navigate('/workflow/dashboard', { replace: true });
    }
  ...
  const selectedMenuKey = currentPath.startsWith('/workflow/workflows') || currentPath.startsWith('/workflow/assigned')
    ? '2'
    : currentPath.startsWith('/workflow/workflow-tracker')
      ? '3'
      : currentPath.startsWith('/workflow/report')
        ? '4'
        : '1';
