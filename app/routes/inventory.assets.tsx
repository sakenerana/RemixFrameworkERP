...
export default function AssetsRoute() {
  ...
  const editRecord = (record: Asset) => {
    navigate(`/inventory/form-asset/${record.id}`);
  };
  ...
  const columns: TableColumnsType<Asset> = [
    {
      ...
      width: 350,
      render: (_, data) => (
        <Link to={`/inventory/asset-tag/${data.id}`} className="flex flex-wrap">
    ...
    <div className="w-full px-6 rounded-lg shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <Space wrap className="mt-2 sm:mt-0">
          <Link to="/inventory/deleted-assets">
            <Button
            ...
            emptyText: (
              <div className="py-8 flex flex-col items-center">
                ...
                <Button
                  type="primary"
                  className="mt-2"
                  onClick={() => navigate('/inventory/form-asset')}
                  icon={<AiOutlinePlus />}
                >
                  Add First Asset
                </Button>
