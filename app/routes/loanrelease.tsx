...
import MetricCardLoanRelease from "~/components/MetricCardLoanRelease";
...
interface LoanReleaseBranch {
    branch_name: string;
    branch_total: number;
    satellites: SatelliteEntry[];
}

interface LoanReleaseBranchResponse {
    error: boolean;
    message: string;
    workflow_name: string;
    year: number;
    overall_total?: number;
    data: LoanReleaseBranch[];
}
...
const getMonthsDivisor = (selectedYear: number) => {
...
export default function LoanReleasesLayoutIndex() {
    ...
    useEffect(() => {
        ...
        const fetchLoanReleaseBranchData = async () => {
            setIsLoading(true);
            setHasError(false);
            ...
            try {
                const response = await axios.get<LoanReleaseBranchResponse>(
                    `${import.meta.env.VITE_API_BASE_URL}/loanprocessingv2/branch-satellite-data/${selectedYear}`,
                    {
                        ...
                        const staffs: Staff[] =
                            sortedSatellites.length > 0
                                ? sortedSatellites.map((satellite, index) => ({
                                      ...
                                      tasks: Number(
                                          (
                                              Number(satellite.satellite_total ?? 0) / monthsDivisor
                                          ).toFixed(1)
                                      ),
    ...
    const content = useMemo(() => {
        if (isLoading) {
            return (
                <div className="rounded-sm border border-gray-300 bg-white px-6 py-10 text-center text-sm text-gray-500 shadow-sm">
                    Loading loan release branch cards...
                </div>
            );
        }
        ...
        if (hasError) {
            return (
                <div className="rounded-sm border border-red-200 bg-red-50 px-6 py-10 text-center text-sm text-red-600 shadow-sm">
                    Unable to load loan release data for {selectedYear}.
                </div>
            );
        }
        ...
        if (branches.length === 0) {
            return (
                <div className="rounded-sm border border-gray-300 bg-white px-6 py-10 text-center text-sm text-gray-500 shadow-sm">
                    No loan release branch data available for {selectedYear}.
                </div>
            );
        }
