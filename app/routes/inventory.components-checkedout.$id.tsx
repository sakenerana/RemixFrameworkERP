...
export default function Checkedout() {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const dataFetch = await ComponentService.getAllChecked(isDepartmentID, Number(id));
    ...
    const handleCheckinButton = (data: Component) => {
        setIsModalOpen(true);
        setDataRow(data);
    };
        ...
        try {
            ...
            if (values.checkin?.toLowerCase() === 'checkin') {
                try {
                    ...
                    const success = await ComponentService.deleteComponentCheck(Number(dataRow.id));
            ...
            render: (_, data) => (
                <div>
                    <Popconfirm
                        ...
                        description="Are you sure to checkin this component?"
                        okText="Yes"
                        cancelText="No"
        ...
        <div>
            ...
            <div className="flex justify-between">
                <Alert
                    message={
                        <span>
                            Note: This is the list of all <strong>{data[0]?.components?.name || 'items'}</strong> item user. Please check closely.
                        </span>
                    }
                    type="info"
                    showIcon
